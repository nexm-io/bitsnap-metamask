...
import { Config } from "../utils/script";
...
export interface OffCKBConfig {
  ...
  lumosConfig: (network: CkbNetwork) => Config;
}
...
const testnetLumosConfig: Config = {
  PREFIX: "ckt",
  SCRIPTS: {
    SECP256K1_BLAKE160: {
      CODE_HASH:
...
const mainnetLumosConfig: Config = {
  PREFIX: "ckb",
  SCRIPTS: {
    SECP256K1_BLAKE160: {
      CODE_HASH:
...
const offCKBConfig: OffCKBConfig = {
  ...
  lumosConfig: (network: CkbNetwork) => {
